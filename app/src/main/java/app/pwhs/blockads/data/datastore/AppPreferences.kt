...
class AppPreferences(private val context: Context) {
    ...
    companion object {
        ...
        private val KEY_ADS_ONLY_MODE = booleanPreferencesKey("ads_only_mode")
...
val filterUrl: Flow<String> = context.dataStore.data.map { prefs ->
    ...
    // Ads-only overlay: when on, non-ad categories (adult/gambling/security)
    // are dropped at filter-load time and SafeSearch/YouTube-restricted are
    // forced off when configuring the engine.
    val adsOnlyMode: Flow<Boolean> = context.dataStore.data.map { prefs ->
        prefs[KEY_ADS_ONLY_MODE] ?: false
    }
    ...
    suspend fun setAdsOnlyMode(enabled: Boolean) {
        context.dataStore.edit { prefs ->
            prefs[KEY_ADS_ONLY_MODE] = enabled
        }
    }
