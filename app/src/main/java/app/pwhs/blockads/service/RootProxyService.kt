    ...
    private fun startProxy(startedFromBoot: Boolean = false) {
        ...
        serviceScope.launch {
            try {
                ...
                var primary = appPrefs.upstreamDns.first()
                var fallback = appPrefs.fallbackDns.first()
                if (appPrefs.dnsProviderId.first() == "system") {
                    // Region-aware "System Default" (see RegionDns)
                    primary = RegionDns.systemPrimary()
                    fallback = RegionDns.systemFallback()
                }
                val dohUrl = appPrefs.dohUrl.first()
                val safeSearch = appPrefs.safeSearchEnabled.first() && !appPrefs.adsOnlyMode.first()
                val youtubeSafe = appPrefs.youtubeRestrictedMode.first() && !appPrefs.adsOnlyMode.first()
                ...
                // Background network sync AFTER the engine is up. The reload +
                // updateTries covers rule changes without touching iptables.
                launch {
